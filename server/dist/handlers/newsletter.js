...
const subscribeNewsletter = async (req, res) => {
    ...
    (0, logger_1.log)('subscribeNewsletter called', { email, referredBy, role });
    if (!email) {
        return res.status(400).json({ error: 'Email is required' });
    }
    try {
        const { rows } = await (0, db_1.query)('SELECT id FROM users WHERE email = $1', [email]);
        if (rows.length > 0) {
            return res.status(409).json({ error: 'Email already exists' });
        }
        (0, logger_1.log)('Email not found in DB, proceeding');
    }
    catch (err) {
        (0, logger_1.error)('Error checking email', err);
        return res.status(500).json({ error: 'Unable to check email' });
    }
    const referralCode = Math.random().toString(36).slice(2, 8).toUpperCase();
    const baseUrl = process.env.FRONTEND_URL || 'http://localhost:5173';
    const referralLink = `${baseUrl}?ref=${referralCode}`;
    (0, logger_1.log)('Generated referral link', referralLink);
    try {
        const qrCodeBuffer = await qrcode_1.default.toBuffer(referralLink);
        const id = (0, crypto_1.randomUUID)();
        await (0, db_1.query)('INSERT INTO users (id, email, first_name, last_name, roles, phone, avatar, referral_code, referred_by, created_at) VALUES ($1,$2,$3,$4,$5,$6,$7,$8,$9,NOW())', [
            id,
            email,
            '',
            '',
        ...
        (0, logger_1.log)('Inserted new user', id);
        ...
        let transporter;
        if (smtpReady) {
            (0, logger_1.log)('Creating SMTP transporter');
            ...
            (0, logger_1.log)('SMTP not configured, skipping email send');
        }
        if (transporter) {
            (0, logger_1.log)('Sending email to', email);
            const htmlTemplate = `
        <!DOCTYPE html>
        <html lang="fr">
        <head>
      ...
      `;
        ...
        (0, logger_1.error)('Error during subscription', err);
